...
const BUILTIN_SLASH_COMMANDS: SlashCommandPaletteItem[] = [
  { name: "compact", description: "压缩上下文，可选择附带指令", source: "builtin" },
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{
                ...
                排队中 · {(queuedMessages?.steering.length ?? 0) + (queuedMessages?.followUp.length ?? 0)}
              </span>
              {onRecallQueue && (
                <button
                  onClick={onRecallQueue}
                  title="移除所有排队消息并将其放回输入框编辑"
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
                ...
                <div style={{ maxHeight: "calc(min(48vh, 400px) - 34px)", overflowY: "auto", padding: 4 }}>
                  {!indexLoading && atMatches.length === 0 ? (
                    <div style={{ padding: "6px 8px", fontSize: 12, color: "var(--text-dim)" }}>
                      {needsServerSearch && !serverResultInUse ? "正在搜索…" : "无匹配文件"}
                    </div>
                  ) : (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0, alignSelf: "flex-end" }}>
              {onSteer && (
                <button
                  ...
                  title={attachedImages.length ? "助手运行时无法排队图片附件" : "中断当前运行并立即注入此消息"}
                  style={{
              ...
              {onFollowUp && (
                <button
                  ...
                  disabled={!canQueueStreamingMessage}
                  title={attachedImages.length ? "助手运行时无法排队图片附件" : "在助手完成后排队此消息"}
                  style={{
                    display: "flex", alignItems: "center", gap: 5,
                    padding: "7px 12px",
            ...
            {!isStreaming && onThinkingLevelChange && (
              <div ref={thinkingDropdownRef} style={{ position: "relative" }}>
                <button
                  ...
                  disabled={isStreaming}
                  title={`更改推理等级：${thinkingDisplayLabel}`}
                  aria-label="更改推理等级"
                  style={{
