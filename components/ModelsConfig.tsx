...
function ProviderDetail({ name, provider, onChange, onRename, onDelete }: {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Field label="提供商名称">
        <TextInput value={editingName} onChange={setEditingName} placeholder="提供商名称" mono />
      ...
      <Field label="API 密钥">
        <SecretTextInput value={provider.apiKey ?? ""} onChange={(v) => set("apiKey", v || undefined)}
          placeholder="环境变量名、!shell-command 或直接密钥" mono />
        <span style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>
          前缀加 <code style={{ fontFamily: "var(--font-mono)" }}>!</code> 以运行 shell 命令，或使用环境变量名
        </span>
      </Field>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <Field label="ID *"><TextInput value={model.id} onChange={(v) => set("id", v)} placeholder="模型ID" mono /></Field>
      ...
      <div>
        <SectionTitle>费用（每百万令牌）</SectionTitle>
        <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8 }}>
          {([
            ...
            <Field key={key} label={label}>
              <NumInput value={costVal(key)} onChange={(v) => setCost(key, v)} placeholder="0" />
            </Field>
          ))}
        </div>
      </div>
...
function OAuthDetail({ provider, onRefresh }: { provider: OAuthProvider; onRefresh: () => void }) {
  ...
  const handleLogin = useCallback(() => {
    ...
    es.onerror = () => {
      es.close();
      setLoginState((prev) => prev.phase === "success" ? prev : { phase: "error", message: "连接丢失" });
    };
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
              {loginState.phase === "auth"
                ? "在浏览器中完成登录，然后从地址栏复制重定向 URL 并粘贴到下方。"
                : loginState.message}
            </p>
            {loginState.phase === "auth" && (
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <input
                ...
                placeholder={loginState.phase === "auth" ? "http://localhost:1455/auth/callback?code=…" : (loginState.placeholder ?? "输入值…")}
...
function ApiKeyDetail({ provider, onRefresh }: { provider: ApiKeyProvider; onRefresh: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
        {provider.configured
          ? `API 密钥已存储。在下方输入新密钥以替换它，或断开连接以删除它。`
    ...
    >
      <div style={{ width: 820, maxWidth: "calc(100vw - 32px)", maxHeight: "min(72vh, calc(100vh - 32px))", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, display: "flex", flexDirection: "column", boxShadow: "0 8px 32px rgba(0,0,0,0.22)", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
          {totalCount === 0 ? (
            <div style={{ padding: "20px 0", fontSize: 12, color: "var(--text-dim)", textAlign: "center" }}>无匹配的提供商</div>
          ) : (
